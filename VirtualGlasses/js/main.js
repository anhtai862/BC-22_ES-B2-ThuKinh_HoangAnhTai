...
const glassList = dataGlasses.map((glass) => {
...
const chooseGlass = () => {
  const showGlassList = glassList.reduce((result, glassObject) => {
    let { src, virtualImg } = glassObject;
    return (result += `
            <div  class="col-4 ">
                <img style="width:150px" src="${src}" onclick="glassType('${virtualImg}')">
            </div>
            `);
  }, " ");
